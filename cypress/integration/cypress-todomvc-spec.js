...
describe('autobots-cypress week3 challenge', () => {
    ...
    it('TodoMVC End to End Tests', () => {
        ...
        // Verifying the ordering of todos
        ...
        cy.get('.todo-list li')
            .should(($ele) => {
                ...
                expect(arrayOfliElements).to.have.ordered.members(todos, 'Check the todos list')
            })
        // #4: Create a third to-do
        createTodo(todo.name3)
        cy.get('.new-todo').should('have.value', '')
        cy.get('span.todo-count').should('contain.text', '3 items left')
        ...
        cy.get('.todo-list li:nth-child(1) .toggle').click()
        cy.get('.todo-list li:nth-child(1) .toggle').should('not.to.be.checked')
        cy.contains('Completed').click()
